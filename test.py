...
def detect_edges_multi_method(gray):
...
def find_best_quad(edge_map, image_area, img_h, img_w, debug_label=""):
    ...
    for i, cnt in enumerate(contours[:8]):  # Chỉ xét 8 contour lớn nhất
        ...
        # Debug: in lý do fail cho contour lớn nhất
        if i == 0:
            peri = cv2.arcLength(cnt, True)
            approx = cv2.approxPolyDP(cnt, 0.02 * peri, True)
            _, reason = is_valid_slide_quad(approx, image_area, img_h, img_w)
            print(f"  [{debug_label}] Largest contour rejected: {reason}")
...
def detect_screen_corners(image_path, debug=True):
    ...
    # Thử từng method detect edge
    edge_methods = detect_edges_multi_method(gray)
...
if __name__ == "__main__":
    ...
    image_path = r"data/Messenger_creation_94686750-6984-4C58-AB29-10C4CEF860DC.jpeg"
    if len(sys.argv) > 1:
